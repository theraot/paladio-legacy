 ...
 * modify it under the terms of the GNU L General Public
 ...
 * You should have received a copy of the GNU L General Public
	...
	public static function resolveClassName($class) {
		...
		foreach(self::getDeclaredAnnotations() as $declared) {
			if($declared == $class) {
				...
				$position = strrpos($declared, "_$class");
				if($position !== false && ($position + strlen($class) == strlen($declared) - 1)) {
					$matching[] = $declared;
				}
			}
		}
